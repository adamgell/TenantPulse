...
if ($SkipTestProof) {
    ...
    # MinimumTests 1169 / -AllowNotRun 1 (Task 4.2, TP.ENT.0006 - see
    ...
    & pwsh -NoProfile -File $gate -ResultPath $TestResultPath -MinimumTests 1169 -AllowedSkips 0 -AllowNotRun 1 | Write-Verbose
    if ($LASTEXITCODE -ne 0) {
        throw "The supplied test result did not pass the whole-result gate, so this package must not be published. Run: pwsh -File tests/QA/Assert-GateResult.ps1 -ResultPath '$TestResultPath' -MinimumTests 1169"
    }
