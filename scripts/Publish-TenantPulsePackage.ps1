...
if ($SkipTestProof) {
    ...
    # MinimumTests 1325 / -AllowNotRun 1 (Task 4.4, TP.ENT.0017-0024 - see
    # .build/AssertGateResult.tasks.ps1's own $script:tenantPulseGateMinimumTests
    # accounting comment for the full per-commit history; this file's own value must
    # stay in sync with that one - see the Task 4.2 note this comment replaces for the
    # earlier drift-and-correction this file already went through once).
    $gate = Join-Path $repoRoot 'tests/QA/Assert-GateResult.ps1'
    & pwsh -NoProfile -File $gate -ResultPath $TestResultPath -MinimumTests 1325 -AllowedSkips 0 -AllowNotRun 1 | Write-Verbose
    if ($LASTEXITCODE -ne 0) {
        throw "The supplied test result did not pass the whole-result gate, so this package must not be published. Run: pwsh -File tests/QA/Assert-GateResult.ps1 -ResultPath '$TestResultPath' -MinimumTests 1325"
    }
