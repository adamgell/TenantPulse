...
if ($SkipTestProof) {
    ...
    # MinimumTests 1154 / -AllowNotRun 1 (Task 4.2, TP.ENT.0006 - see
    # .build/AssertGateResult.tasks.ps1's own $script:tenantPulseGateMinimumTests
    # accounting comment for the full per-commit history; this file's own value had
    # drifted out of sync with that one - stuck at 713 since Task 1.11 - and is corrected
    # here to match rather than compounded further).
    $gate = Join-Path $repoRoot 'tests/QA/Assert-GateResult.ps1'
    & pwsh -NoProfile -File $gate -ResultPath $TestResultPath -MinimumTests 1154 -AllowedSkips 0 -AllowNotRun 1 | Write-Verbose
    if ($LASTEXITCODE -ne 0) {
        throw "The supplied test result did not pass the whole-result gate, so this package must not be published. Run: pwsh -File tests/QA/Assert-GateResult.ps1 -ResultPath '$TestResultPath' -MinimumTests 1154"
    }
