@{
    ...
    Data       = @{
        ...
        PartialDatasets = @('endpointSecurityLapsPolicies')
        Gates           = @('Intune')
    }
    Rule       = @{
        Type     = 'Function'
    ...
    Consulting = @{
        WhatItMeans  = 'Windows LAPS (Local Administrator Password Solution), configured through Intune''s dedicated Endpoint Security > Account protection blade, randomizes and backs up each device''s local administrator password to Microsoft Entra ID rather than leaving it static or shared across the fleet. This check asserts a minimum security bar on a SINGLE policy: backs up to Entra ID, 4-class-or-higher password complexity, at least 14-character passwords, and a recognized post-authentication action (password rotates after use/unlock/expiry) - all four on the same policy, never mixed across separate policies. NOTE: 14 characters is THIS CHECK''S OWN chosen bar (matching Windows LAPS''s own configuration-time default), not a platform-enforced floor - the underlying BitLocker/LAPS CSP itself accepts any password length from 8 to 64 characters; a shorter, CSP-valid password will still Fail this specific check''s stricter bar. Likewise "no post-authentication action configured" means the policy has no value set for that setting at all (the setting row is absent from the resolved policy), not that the resolved value equals some literal "Not configured" enum member.'
        ...
        Remediation  = @(
            'Intune admin center > Endpoint security > Account protection > Create Policy (Windows 10 and later, Local admin password solution (Windows LAPS) profile) - set Backup directory to Microsoft Entra ID (Azure AD only), password complexity to 4-class or higher, password length to 14 or more characters (this check''s own bar, not a CSP-enforced minimum), and a post-authentication action that is actually set to a real value (e.g. rotate on unlock) rather than left unconfigured.'
