@{
    ...
    Consulting = @{
        WhatItMeans  = 'Intune Company Portal branding (Tenant Administration > Customization) controls the organization name, theme color, logo, and privacy statement URL end users see across the Company Portal apps, Company Portal website, and the Android Intune app - including during enrollment. Every tenant has a default branding profile; up to 25 additional group-targeted profiles can also be created. This check Passes when the default profile has a non-blank organization name or privacy URL, OR when a custom non-default profile has an effective include assignment. Custom-profile existence without assignment evidence is not treated as deployed branding.'
