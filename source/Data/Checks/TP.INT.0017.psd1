@{
    ...
    Consulting = @{
        WhatItMeans  = 'App Control for Business (formerly WDAC) restricts which applications and drivers are allowed to run. This check reads Part A''s setting-presence index and Passes only when at least one confirmed-assigned App Control policy is in Enforce mode (audit mode disabled) AND has an active control - built-in controls selected, or a custom XML upload whose payload is non-empty. Evaluated as a SAME-POLICY AND, not a tenant-wide union: a tenant that audits on policy A and uploads empty XML on policy B still Fails, because neither policy actually blocks untrusted executables. Unknown or unassigned policies cannot make the check Pass.'
