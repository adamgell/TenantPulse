@{
    ...
    Consulting = @{
        WhatItMeans  = 'Managed Installer automatically trusts applications deployed through Intune (or ConfigMgr) so they are not blocked by App Control. This check reads Part A''s setting-presence index and Passes only when "Trust apps from managed installer" is enabled on a confirmed-assigned policy that is itself in Enforce mode with an active control - the same bar TP.INT.0017 uses. Evaluated as a SAME-POLICY AND: Managed Installer on an audit-only policy, on an enforce-mode upload with an empty XML payload, or on a policy without a confirmed positive assignment target Fails, because the underlying App Control policy is not actually deployed and enforcing the intended trust path.'
