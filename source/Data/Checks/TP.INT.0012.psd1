@{
    ...
    Consulting = @{
        WhatItMeans  = 'Windows Feature Update deployment profiles (a distinct beta resource from the update-ring/deferral profiles TP.INT.0004 already checks) pin a device population to a specific Windows feature update VERSION - e.g. "Windows 11, version 22H2". Each Windows version has a published end-of-support date after which Microsoft stops shipping security updates for it. This check Fails when any effectively assigned Feature Update profile targets a version whose end-of-support date has already passed as of when the snapshot was collected; profiles without authoritative assignment evidence cannot produce a false Pass or Fail.'
