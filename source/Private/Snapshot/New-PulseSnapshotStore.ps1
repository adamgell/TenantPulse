<#
    ...
    SCHEMA 2.0.0 (Task 1): the manifest carries the structured provider outcome fields in
    each dataset entry and initializes the `references`/`expansions` namespaces for later
    reference and expansion writers. Both namespaces reuse Set-PulseManifestEntry's own
    mutex/atomic tmp+rename machinery rather than forking a second read-modify-write path.
    scoringModelVersion still belongs to the findings document, not the snapshot; that
    boundary has not moved. A snapshot opened via Get-PulseSnapshotStore that declares
    schemaVersion 1.0.0 or 1.1.0 is migrated in memory for structured dataset reads while
    retaining its declared schema and namespace capability. In particular, a 1.0.0 store
    has no `references`/`expansions` members and remains ineligible for those writes.
